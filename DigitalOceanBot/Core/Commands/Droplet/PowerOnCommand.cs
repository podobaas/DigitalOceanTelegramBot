...
{
    [BotCommand(BotCommandType.DropletPowerOn)]
    public sealed class PowerOnCommand : IBotCommand
    {
        ...
        public async Task ExecuteCommandAsync(Message message)
        {
            ...
            if (dropletId > 0)
            {
                ...
                var action = await _digitalOceanClient.DropletActions.PowerOn(dropletId);
