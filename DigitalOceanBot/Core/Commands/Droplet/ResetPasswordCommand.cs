...
{
    [BotCommand(BotCommandType.DropletResetPassword)]
    public sealed class ResetPasswordCommand : IBotCommand
    {
        ...
        public async Task ExecuteCommandAsync(Message message)
        {
            ...
            if (dropletId > 0)
            {
                ...
                var action = await _digitalOceanClient.DropletActions.ResetPassword(dropletId);
